...
struct
  ...
  in
    ...
    and show_pat_ ctx pat_ =
      ...
      | Pparens pat => parensAround (show_pat ctx pat)
      | Punit => text "()"
      | Ptuple pats =>
      ...
      | Pinfix {left, id, right} =>
          parensAround (
            show_pat ctx left +-+ show_id id +-+ show_pat ctx right
          )
      | Ptyped {pat, ty} =>
          parensAround (
            show_pat ctx pat
  ...
  in
    ...
    and show_exp_ ctx exp_ =
      ...
      | Eparens (exp as Efn _) => show_exp exp
      ...
      | Einfix {left, id, right} =>
          group (
              show_atexp left $$ (show_id id +-+ show_atexp right)
          )
      | Etyped {exp, ty} =>
          ...
          | hd::tl =>
              group (
                parensAround (
                  ...
                  $$
                  spaces 2 ++ (show_match ctx) hd
                  $$
                  show_list_base false (show_match ctx) "| " true true tl
                )
              )
          )
      | Efn matches =>
    ...
    and show_atexp ctx exp =
      ...
      | ( Eident {opp = true, ...}
        ...
        | Eif _
        | Ewhile _
        | Ecase _ ) => parensAround (show_exp ctx exp)
  ...
  in
    ...
    val report = fn ctx => fn exp => fn n => fn location =>
      PrettySimpleDoc.toString true (report ctx empty_set (show_exp ctx exp) n location)
  end
  (*
  local
    open Context
